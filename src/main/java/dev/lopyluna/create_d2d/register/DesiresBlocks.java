...
public class DesiresBlocks {
    ...
    public static final DyedBlockList<Block> DYED_ASPHALT_BLOCK = new DyedBlockList<>(color -> {
        ...
        return REG.block(colorName + "_asphalt", Block::new)
                ...
                .blockstate((c, p) -> {
                    var model = p.models().withExistingParent(colorName + "_asphalt", p.modLoc("block/asphalt"))
                            .texture("all", p.modLoc("block/asphalt/" + colorName));
                    p.simpleBlockWithItem(c.get(), model);
                })
                .item()
    ...
    public static final DyedBlockList<PropellerBlock> DYED_PROPELLERS = new DyedBlockList<>(color -> {
        ...
        return REG.block(colorName + "_propeller", p -> new PropellerBlock(p, color))
                ...
                .item()
                .tag(DesiresTags.ItemTags.DYED_BLOCKS.tag)
    ...
    public static void fanSailCrafting(ItemLike itemLike, ItemLike catalyst, RegistrateRecipeProvider pFinishedRecipeConsumer, DataGenContext<Block, FanSailBlock> c) {
        ShapedRecipeBuilder.shaped(RecipeCategory.BUILDING_BLOCKS, itemLike, 4)
                ...
                .save(pFinishedRecipeConsumer, DesiresCreate.loc("crafting/fan_catalyst/" + c.getName()));
    }
