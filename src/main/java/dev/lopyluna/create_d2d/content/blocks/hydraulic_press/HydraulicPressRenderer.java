...
public class HydraulicPressRenderer extends KineticBlockEntityRenderer<HydraulicPressBE> {
    ...
    protected void renderFluid(HydraulicPressBE be, float partialTicks, PoseStack ms, MultiBufferSource buffer, int light) {
        ...
        if (!fluidStack.isEmpty() && level != 0) {
            if (!(level < 1f / 256f)) {
                float clampedLevel = Mth.clamp(level * 0.5f, 0, 0.5f);
                ...
                ms.pushPose();
                ms.translate(0, clampedLevel - 0.5f, 0);
                FluidRenderer.renderFluidBox(fluidStack.getFluid(), fluidStack.getAmount(), 0.0703125f, yMin, 0.0703125f, 0.9296875f, yMax, 0.9296875f,
