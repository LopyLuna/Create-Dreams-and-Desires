...
public class PropellerVisual extends KineticBlockEntityVisual<PropellerBE> implements SimpleDynamicVisual {
    ...
    public PropellerVisual(VisualizationContext context, PropellerBE blockEntity, float partialTick) {
        ...
        shaft.setup(PropellerVisual.this.blockEntity).setPosition(getVisualPosition()).rotateToFace(axis).setChanged();

        BlockState state = blockEntity.getBlockState();
        DyeColor color = null;
        if (state.getBlock() instanceof PropellerBlock block) color = block.color;
        wheel = instancerProvider().instancer(InstanceTypes.TRANSFORMED, Models.partial(color == null ? DesiresPartialModels.PROPELLER : DesiresPartialModels.DYED_PROPELLERS.get(color)))
                .createInstance();
