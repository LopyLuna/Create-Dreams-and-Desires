...
public class NormalLogRotatedBlockPillar extends RotatedPillarBlock {
    ...
    public BlockState getToolModifiedState(BlockState state, UseOnContext context, ToolAction toolAction, boolean simulate) {
        if(context.getItemInHand().getItem() instanceof AxeItem) {
            ...
            if(state.is(DDBlocks.rubber_log.get())) {
                return DDBlocks.stripped_rubber_log.get().defaultBlockState().setValue(AXIS, state.getValue(AXIS));
            }
            if(state.is(DDBlocks.rubber_wood.get())) {
                return DDBlocks.stripped_rubber_wood.get().defaultBlockState().setValue(AXIS, state.getValue(AXIS));
            }
        }
