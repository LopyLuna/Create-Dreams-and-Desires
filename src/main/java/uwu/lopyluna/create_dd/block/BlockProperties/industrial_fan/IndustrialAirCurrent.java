...
public class IndustrialAirCurrent {
    ...
    protected void tickAffectedEntities(Level world) {
        for (Iterator<Entity> iterator = caughtEntities.iterator(); iterator.hasNext();) {
            ...
            InterfaceIndustrialProcessingType processingType = getTypeAt((float) entityDistance);

            if (processingType == IndustrialTypeFanProcessing.NONE)
                continue;
            ...
            if (entity instanceof ItemEntity itemEntity) {
                ...
                if (DDFanProcessing.canProcess(itemEntity, processingType))
                    if (DDFanProcessing.applyProcessing(itemEntity, processingType)
    ...
    public void findAffectedHandlers() {
        ...
        for (int i = 1; i <= limit; i++) {
            InterfaceIndustrialProcessingType segmentType = getTypeAt(i - 1);
