...
public class UnchargedStargazeSingularity extends Item {
    ...
    public boolean onEntityItemUpdate(ItemStack stack, ItemEntity entity) {
        ...
        boolean maxTime = gameTime < DDConfigs.server().stargaze_singularity_max_time.get();
        boolean minTime = gameTime > DDConfigs.server().stargaze_singularity_min_time.get();
        boolean nightTime = maxTime && minTime;

        if (y > (maxHeight / DDConfigs.server().stargaze_singularity_max_height_division.get()) && nightTime && DDConfigs.server().stargaze_singularity_recipe.get()) {
