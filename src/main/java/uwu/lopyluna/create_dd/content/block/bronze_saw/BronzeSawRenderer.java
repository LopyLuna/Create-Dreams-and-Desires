package uwu.lopyluna.create_dd.content.block.bronze_saw;
...
public class BronzeSawRenderer extends SafeBlockEntityRenderer<BronzeSawBlockEntity> {
	...
	protected void renderBlade(BronzeSawBlockEntity be, PoseStack ms, MultiBufferSource buffer, int light) {
		...
		if (BronzeSawBlock.isHorizontal(blockState)) {
			if (speed > 0) {
				partial = DDPartialModels.BRONZE_SAW_BLADE_HORIZONTAL_ACTIVE;
			} else if (speed < 0) {
				partial = DDPartialModels.BRONZE_SAW_BLADE_HORIZONTAL_REVERSED;
			} else {
				partial = DDPartialModels.BRONZE_SAW_BLADE_HORIZONTAL_INACTIVE;
			}
		} else {
			if (be.getSpeed() > 0) {
				partial = DDPartialModels.BRONZE_SAW_BLADE_VERTICAL_ACTIVE;
			} else if (speed < 0) {
				partial = DDPartialModels.BRONZE_SAW_BLADE_VERTICAL_REVERSED;
			} else {
				partial = DDPartialModels.BRONZE_SAW_BLADE_VERTICAL_INACTIVE;
			}
	...
	public static void renderInContraption(MovementContext context, VirtualRenderWorld renderWorld,
		...
		Vec3 facingVec = Vec3.atLowerCornerOf(context.state.getValue(FACING)
				.getNormal());
		...
		if (BronzeSawBlock.isHorizontal(state)) {
			if (shouldAnimate)
				superBuffer = CachedBufferer.partial(DDPartialModels.BRONZE_SAW_BLADE_HORIZONTAL_ACTIVE, state);
			else
				superBuffer = CachedBufferer.partial(DDPartialModels.BRONZE_SAW_BLADE_HORIZONTAL_INACTIVE, state);
		} else {
			if (shouldAnimate)
				superBuffer = CachedBufferer.partial(DDPartialModels.BRONZE_SAW_BLADE_VERTICAL_ACTIVE, state);
			else
				superBuffer = CachedBufferer.partial(DDPartialModels.BRONZE_SAW_BLADE_VERTICAL_INACTIVE, state);
		}
		...
		if (!BronzeSawBlock.isHorizontal(state)) {
			superBuffer.rotateZ(state.getValue(BronzeSawBlock.AXIS_ALONG_FIRST_COORDINATE) ? 90 : 0);
		}
