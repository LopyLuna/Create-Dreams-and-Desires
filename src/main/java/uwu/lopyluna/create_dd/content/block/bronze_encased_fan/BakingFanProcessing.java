...
public class BakingFanProcessing {
	...
	public static boolean isFreezable(ItemStack stack, Level world) {
		...
		Optional<FreezingRecipe> recipe = DDRecipeTypes.FREEZING.find(FREEZING_WRAPPER, world);
		return recipe.isPresent();
	}
	public static boolean isSuperHeatable(ItemStack stack, Level world) {
		SUPERHEATING_WRAPPER.setItem(0, stack);
		Optional<SuperHeatingRecipe> recipe = DDRecipeTypes.SUPERHEATING.find(SUPERHEATING_WRAPPER, world);
		return recipe.isPresent();
	}
		...
		FREEZING {
			...
			public void affectEntity(Entity entity, Level level) {
				...
				if (entity instanceof Skeleton skeleton) {
					int progress = skeleton.getExtraCustomData()
